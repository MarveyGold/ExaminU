...
import mongoose from '$lib/server/db.js'; // reuse existing DB connector
...
export async function GET({ url }) {
  ...
  const filePath = path.join(process.cwd(), 'static', 'data', 'faculties.json');
  try {
    ...
    try {
      const res = await Faculty.insertMany(docs, { ordered: false });
